...
class QuizViewState extends State<QuizView> {
  ...
  Widget initQuizComponents(int index) {
    if (index < questions.length)
      return QuizComponent(question: questions[index], parent: this);
    else
  ...
  void initState() {
    ...
    super.initState();
    if (MyApp.isWebDevice) print("weshalor");
  }
...
class QuizComponentState extends State<QuizComponent> {
  ...
  FlatButton answerButton(Text text) {
    return FlatButton(
        onPressed: () {
      ...
      Column(
        children: [
          Row(
            ...
            children: [
              answerButton(Text(question.answers[0],
                  style: TextStyle(color: Colors.white))),
              SizedBox(width: 25),
              answerButton(Text(question.answers[1],
                  style: TextStyle(color: Colors.white)))
            ],
          ),
          SizedBox(height: 25),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              answerButton(Text(question.answers[3],
                  style: TextStyle(color: Colors.white))),
              SizedBox(width: 25),
              answerButton(Text(question.answers[2],
                  style: TextStyle(color: Colors.white))),
            ],
          )
        ],
